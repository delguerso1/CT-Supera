...
const GerenciarTurmasScreen: React.FC<NavigationProps> = ({ navigation, embedded }) => {
  ...
  const primeiroProfessorId = (turma: Turma): number | null => {
    const p = turma.professor;
  ...
  const ctIdDaTurma = (turma: Turma): number => {
    const c = turma.ct as unknown;
  ...
  const handleEditarTurma = (turma: Turma) => {
    setFormData({
      ...
      professor: primeiroProfessorId(turma),
      aceita_kids: turma.aceita_kids ?? false,
      aceita_teen: turma.aceita_teen ?? false,
  ...
  const handleSalvarTurma = async () => {
    const ctId = Number(formData.ct);
    const cap = Number(formData.capacidade_maxima);
    const horario = String(formData.horario || '').trim();
    if (!ctId || ctId <= 0) {
      Alert.alert('Validação', 'Selecione o centro de treinamento.');
      return;
    }
    if (!horario) {
      Alert.alert('Validação', 'Informe o horário (ex.: 18:00).');
      return;
    }
    if (!formData.dias_semana?.length) {
      Alert.alert('Validação', 'Selecione pelo menos um dia da semana.');
      return;
    }
    if (!cap || cap < 1) {
      Alert.alert('Validação', 'Informe a capacidade máxima (mínimo 1).');
      return;
    }
    ...
    try {
      setSaving(true);
      /** API Django: campo M2M é `professores`, não `professor` */
      const dataToSave = {
        ct: ctId,
        horario,
        ...
        professores: formData.professor ? [formData.professor] : [],
      ...
      if (d && typeof d === 'object') {
        ...
        if (Array.isArray(dias) && dias[0]) msg = String(dias[0]);
        else if (typeof d.horario === 'string') msg = d.horario;
        else if (Array.isArray(d.horario) && d.horario[0]) msg = String(d.horario[0]);
        else {
          const pair = Object.entries(d).find(
            ([k, v]) => k !== 'detail' && Array.isArray(v) && (v as string[])[0]
          );
          if (pair) msg = String((pair[1] as string[])[0]);
        }
      }
  ...
  const toggleDiaSemana = (diaId: number) => {
    const diasAtuais = (formData.dias_semana || []) as number[];
          ...
          <View style={styles.modalContent}>
            ...
            <ScrollView>
              ...
              <View style={styles.diasContainer}>
                {diasSemana.map((dia) => {
                  const diasSel = (formData.dias_semana || []) as number[];
                  const marcado = diasSel.includes(dia.id);
                  return (
                  <TouchableOpacity
                    key={dia.id}
                    style={[
                      styles.diaButton,
                      marcado && styles.diaButtonSelected,
                    ]}
                  ...
                  >
                    <Text
                      style={[
                        ...
                        marcado && styles.diaButtonTextSelected,
                      ]}
                    >
                      {dia.nome}
                    </Text>
