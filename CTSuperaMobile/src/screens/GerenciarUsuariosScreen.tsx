...
const GerenciarUsuariosScreen: React.FC<NavigationProps> = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      if (isPrecadastroLikeTab(activeTab)) {
        ...
        if (activeTab === 'exalunos') precParams.origem = 'ex_aluno';
  ...
  const handleSuspenderContrato = (aluno: User) => {
    Alert.alert('Suspender contrato', 'Escolha a duração da suspensão:', [
      ...
      { text: '30 dias', onPress: () => confirmarSuspensao(aluno, 30) },
      { text: '60 dias', onPress: () => confirmarSuspensao(aluno, 60) },
    ]);
  };
        ...
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert('Erro', error.response?.data?.error || 'Erro ao suspender.');
            }
          },
        },
      ]);
      ...
      { text: 'Cancelar', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            ...
            Alert.alert('Erro', error.response?.data?.error || 'Erro ao reativar.');
          }
        },
      },
    ]);
  ...
  const handleFinalizarMatricula = async () => {
    ...
    if (!matriculaForm.dia_vencimento || Number.isNaN(diaVenM) || diaVenM < 1 || diaVenM > 31) {
      Alert.alert('Atenção', 'Informe o dia de vencimento das mensalidades (entre 1 e 31).');
      return;
    }
    ...
    try {
      ...
      await fetchUsers();
      const msg = 'Pré-cadastro matriculado com sucesso!';
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <ScrollView>
              ...
              <Text style={styles.sectionTitle}>Dia de vencimento das mensalidades (1 a 31)</Text>
              <TextInput
                style={styles.input}
                placeholder="Ex: 7"
                keyboardType="numeric"
                value={matriculaForm.dia_vencimento}
                onChangeText={(value) => setMatriculaForm(prev => ({ ...prev, dia_vencimento: value.replace(/\D/g, '').slice(0, 2) }))}
              />
              ...
              {matriculaForm.ja_aluno ? (
                ...
                <>
                  ...
                  <Text style={{ fontSize: 12, color: '#666', marginTop: 4, marginBottom: 8 }}>
                    A 1ª parcela é lançada no financeiro; cobrança e baixa são feitas pelo gerente no sistema.
                  </Text>
                </>
              )}
              <Text style={styles.sectionTitle}>Dias habilitados para treino</Text>
